...
/// <summary>
/// A Mediator stream pipeline behavior that executes request authorization before invoking the next handler.
/// </summary>
/// <typeparam name="TRequest">The request type.</typeparam>
/// <typeparam name="TResponse">The response type.</typeparam>
public sealed class RequestAuthorizationStreamPipelineBehavior<TRequest, TResponse>
    : IStreamPipelineBehavior<TRequest, TResponse>
    where TRequest : IStreamMessage
{
    ...
    public IAsyncEnumerable<TResponse> Handle(TRequest message, StreamHandlerDelegate<TRequest, TResponse> next, CancellationToken cancellationToken)
    {
        return _corePipelineStep.Handle(message, token => next(message, token), cancellationToken);
    }
}
